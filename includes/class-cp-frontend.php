...
class CP_Frontend {
	...
	/** The disclosure text from Settings (or the built-in default). */
	public static function disclosure_text() {
		$text = get_option( 'cp_disc_text', '' );
		return '' !== $text ? $text : self::default_disclosure_html();
	}

	/** Has this candidate accepted the disclosure? Honors acceptances
	 *  recorded under the brief per-election system as well. */
	public static function candidate_accepted( $candidate_id ) {
		if ( get_post_meta( $candidate_id, '_cp_disclosure_date', true ) ) {
			return true;
		}
		foreach ( array_map( 'intval', (array) get_post_meta( $candidate_id, '_cp_elections', true ) ) as $eid ) {
			if ( get_post_meta( $candidate_id, '_cp_disclosure_accept_' . $eid, true ) ) {
				return true;
			}
		}
		return false;
	...
	/** Should this candidate appear on public lists? */
	public static function candidate_visible( $candidate_id ) {
		...
		return self::candidate_accepted( $candidate_id );
	}
	...
	public static function admin_disclosure_status( $candidate_id, $detailed = false ) {
		...
		if ( ! self::disclosure_required() ) {
			return $detailed ? 'disclosure is turned off in Settings' : '&#10003;';
		}
		if ( self::candidate_accepted( $candidate_id ) ) {
			$date = get_post_meta( $candidate_id, '_cp_disclosure_date', true );
			return $detailed ? ( 'accepted' . ( $date ? ' ' . esc_html( date_i18n( get_option( 'date_format' ), strtotime( $date ) ) ) : '' ) ) : '&#10003;';
		}
		return $detailed ? '<em>not yet - the candidate must accept it in their portal</em>' : '—';
	}
	...
	private static function render_election( $election, $with_heading ) {
		...
		$candidates = array_values( array_filter( $candidates, function ( $c ) use ( $election ) {
			...
			return self::candidate_visible( $c->ID );
		} ) );
	...
	public static function shortcode_portal() {
		...
		<div class="cp-portal">
			...
			<form method="post" enctype="multipart/form-data">
				...
				<?php if ( self::disclosure_required() ) :
					$disc_date = get_post_meta( $post->ID, '_cp_disclosure_date', true );
					$accepted  = self::candidate_accepted( $post->ID );
				?>
				<div class="cp-disclosure">
					<h4><?php esc_html_e( 'Candidate Disclosure Statement', 'candidate-portal' ); ?></h4>
					<p><?php echo wp_kses_post( self::disclosure_text() ); ?></p>
					<p><label class="cp-inline"><input type="checkbox" name="disclosure" value="1" required <?php checked( $accepted ); ?> /> <?php esc_html_e( 'I agree (required)', 'candidate-portal' ); ?></label>
					<?php if ( $accepted && $disc_date ) : ?>
	...
	public static function maybe_handle_profile_save() {
		...
		// Disclosure: record the acceptance date the first time it is
		// accepted. It is a historical record the candidate cannot undo.
		if ( ! empty( $_POST['disclosure'] ) && ! get_post_meta( $post->ID, '_cp_disclosure_date', true ) ) {
